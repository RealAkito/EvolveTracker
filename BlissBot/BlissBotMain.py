...
def RegexReceiver(update, context):
    ...
    if update.message and update.message.text:
        ...
        if m:
            ...
            link = Markdown.EscapeMarkdownLink(
                reverse('bugs:ticketuuid', kwargs={'uuid': issue.issueuuid}))
            device = Markdown.EscapeMarkdown(issue.device)
            updated = Markdown.EscapeMarkdown(
                issue.updated.strftime("%a, %b %-d %Y %H:%M"))

            # Our actual message
            msg = f"\N{Lady beetle} *Bug Report* from _{issue.username}_\n\n"
            msg += "```\n" + Markdown.EscapeMarkdownCode(
                x.get_string()) + "\n```\n"
            msg += f"*Device*: {device}\n*Modified*: {updated}\n\n"
...
def MessageReceiver(update, context):
    ...
    if update.message:
        logger.info(
            f"From User: {update.message.from_user.username} - {update.message.from_user.id}"
        )
...
def main():
    ...
    Conch.CreateManhole(application,
                        ({
                            'updater': updater,
                            'website': Delivery.InitializeDelivery(application)
                        }, ))
