...
class Issue(models.Model):
	...
	status = models.IntegerField(verbose_name=u"Status", choices=STATUS_CHOICES, default=1)
	# Our resolution choices
	resolution = models.IntegerField(verbose_name=u"Resolution", choices=RESOLUTION_CHOICES, null=True, blank=True)
	# Our type choices (`type` is a reserved keyword in python so we use issuetype instead)
	issuetype = models.IntegerField(verbose_name=u"Type", choices=TYPE_CHOICES, default=0)
	# Our Severity Choices
	severity = models.IntegerField(verbose_name=u"Severity", choices=SEVERITY_CHOICES, default=0)
	# Whether the user is rooted or not
	rooted = models.BooleanField(verbose_name=u"Device is Rooted", default=False)
